...
Image media in **Audience in Space** also exposes a local **Spatial upscale** round trip. Zenith captures six gaze-oriented 110° perspective crops from the exact measured audience position, with 20° overlap and padded borders, and downloads them as one 3×2 PNG atlas. On angular domes, the forward crop is tilted from the observer so its lower center lands on the actually visible carrier rim; the front view no longer wastes crop resolution below the source field, while the otherwise-unused opposite polar face retains the original front orientation as seam coverage. Upscale that complete atlas in any external tool, then import it back without cropping. Zenith reprojects every crop through the pinned physical carrier, solves bounded exposure gains in shared regions, and reconstructs the original carrier map with linear-light Laplacian pyramid blending. The resulting `Spatial master` is added directly to Review; no paid provider call occurs. Both the atlas and result carry portable Zenith PNG metadata for the projection, audience pose, source identity, scale, and stitch parameters. If an external editor strips ancillary PNG metadata, the same browser session retains the most recent capture contract as a fallback.
