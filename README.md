...
Curated pairs and trios can also be prepared as a **Fulldome bundle**. Choose which visible source owns the zenith slot, apply the captured Plate 03 profile or its spatial mirror, refine the still-editable placement if necessary, and use the normal exact PNG export. The same operation is available through a local [headless fulldome composition](docs/fulldome-image-pipeline.md) API and CLI; it does not require opening the Workbench, classify source content, or invoke an image model.
...
Image media in **Audience in Space** also exposes a local **Spatial upscale** round trip. For angular domes, Zenith captures five yaw-distributed 110° views around the visible rim plus one polar view from the exact measured audience position. Each rim view is reversibly normalized against the projected carrier boundary, so every exported square is filled with useful image data and neighboring squares retain 38° of horizontal overlap. The downloaded ZIP contains six independent metadata-bearing PNGs, a reference 3×2 atlas, and a JSON manifest. The recommended 4K workflow captures 1024² crops, upscales every square independently to 1920² without cropping or renaming, and imports all six together; Zenith pins the reconstructed angular master to the exact 4096×4096 fulldome delivery raster instead of letting the external tile scale determine its dimensions. The legacy whole-atlas import remains accepted. Zenith inverts each tile normalization, reprojects every crop through the pinned physical carrier, solves bounded exposure gains in shared regions, and reconstructs the original carrier map with linear-light Laplacian pyramid blending. The resulting `Spatial master` is added directly to Review; no paid provider call occurs. The tiles, atlas, and result carry portable Zenith metadata for the projection, audience pose, source identity, scale, and stitch parameters. If an external editor strips ancillary PNG metadata, the filenames plus the same browser session retain the most recent capture contract as a fallback; the bundled JSON manifest can restore that contract in a later session.

Review also exposes **Immersive Preview** from the same pinned spatial specification. Phone Lookaround converts the complete W3C `alpha`/`beta`/`gamma` pose into a relative quaternion, recenters without claiming an absolute compass heading, and applies time-based adaptive SLERP plus spike limiting before deriving a complete camera basis. Forward, up, and physical roll remain continuous through the zenith, the rear hemisphere, and arbitrary Z rotation instead of snapping the view upright at portrait/landscape boundaries. One-finger drag remains a universal orientation fallback; pinch, wheel, and overlay controls move the observer forward or backward through the carrier with a bounded world-space offset. A phone gyroscope does not provide stable positional tracking, so physical translation is supplied only by compatible WebXR devices. Compatible WebXR headsets enter the carrier at 1:1 scale from the authored audience X/Z/facing; the physical headset supplies head height and local movement. Compatible AR devices place a bounded carrier scale model using hit testing. XR poses, touch offsets, placement candidates, frame loops, and controller state are runtime-only and are never written into the project.
...
- Spatial upscale currently accepts still images. Upscaling is deliberately external; Zenith performs the deterministic capture, validation, reprojection, exposure compensation, and stitch locally.
