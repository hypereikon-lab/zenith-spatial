...
Curated pairs and trios can also be prepared as a **Fulldome bundle**. Choose which visible source owns the zenith slot, apply the captured Plate 03 profile or its spatial mirror, refine the still-editable placement if necessary, and use the normal exact PNG export. The same operation is available through the narrow [fulldome image preparation](docs/fulldome-image-pipeline.md) runtime boundary for browser automation; it never classifies source content or invokes an image model.
