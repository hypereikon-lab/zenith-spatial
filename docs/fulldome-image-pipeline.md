...
### Content-preserving domemaster reprojection

The default Zenith 180/230 prompt separates two invariants that must not be conflated: the Plate Sketch imagery is
the authoritative visual payload, while the equidistant polar field is the authoritative coordinate system. The
same supplied scene instance, viewpoint evidence, landmarks, internal topology, textures, illumination, and local
relationships must survive. Plate coordinates and rectangular proportions may change only as required to transport
that information through a smooth reprojection.

This distinction is intentional. GPT Image may return a provider-native square resolution rather than the Plate
Sketch raster, so one-to-one `(x,y)` registration is unavailable. Literal registration encourages wide sources to
survive as horizontal bands and can turn the zenith field into an elliptical aperture; unlimited semantic freedom
causes the opposite failure, replacing the supplied views with plausible but newly invented scenery. The contract
therefore allows coordinate deformation but not content substitution. New synthesis is restricted to the missing
guide field, continuation beyond source boundaries, and the narrowest seam-integration band.

The explicit `strict` generation strategy remains available when pixel conservation is genuinely required. Other
carrier families retain their existing integrated contracts; this content-preserving behavior is specific to zenith
domemasters.
