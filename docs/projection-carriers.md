...
| Per-image history                   | `ImageSpatialSpec` on immutable image revisions                      | The projection, fit, guides, orientation, and target geometry of that exact image.                   |
...
Projection metadata belongs to every immutable image revision. Project review resolves the selected Composition's active `ImageSpatialSpec`, so independent images may use different projection modes, measured surfaces, carrier rasters, field allocations, fits, rotations, and horizon positions without inheriting the current editor setting. Carrier-aware conform preserves a true angular pixel circle while stretching normalized CAVE/cylinder/hall UV sampling across a rectangular raster. It does not relabel topology, measured surface, or carrier anchors: those are authored upstream and require a regenerated Plate/image.

Paid image generation pins a versioned provenance contract before confirmation: the owning Composition, exact Plate Sketch revision, carrier raster, complete `ImageSpatialSpec`, operator, and model. The Plate handoff renderer must return a canvas with that exact raster, and the browser decodes the submitted image bytes before confirmation rather than trusting its filename or metadata. The durable job journal echoes the contract into the result; capture decodes the returned image and refuses attachment if its width or height differs. The clean-image revision inherits spatial truth from the pinned Plate parent rather than from the mutable artifact adapter or current editor selection. If the Plate parent changes while generation is running, Zenith refuses to attach the returned image to the newer Plate. Changing the selected Cue is safe: the result attaches to its originating Composition without replacing the newly selected Composition's workspace media.

Generated PNG revisions also carry the same contract in an uncompressed `zenith.spatial.v1` PNG `iTXt` chunk inserted without decoding or re-encoding pixel data. The immutable revision and durable job remain authoritative; embedded PNG metadata is the portable copy used to recover projection state on re-import. A PNG without Zenith metadata inherits the explicitly selected Composition contract and reports that fallback instead of silently claiming a default projection. Prompt language still governs model behavior, but it is never parsed as machine provenance.
...
7. Persist the mode on each image revision and verify save/load.
